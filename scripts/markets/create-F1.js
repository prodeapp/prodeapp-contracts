...
const marketData = {
  ...
  questions: [
    buildQuestionSingleSelect(`Who will set the fastest time in the Q3 at ${marketName}?`, allDrivers, toTimestamp("2022-10-01 10:00:00 GMT-3") + F1_RACE_DURATION, 'F1'),
    buildQuestionPosition(1, allDrivers, marketName, openingTS),
    buildQuestionPosition(2, allDrivers, marketName, openingTS+1),
    buildQuestionPosition(3, allDrivers, marketName, openingTS+2),
    buildQuestionSingleSelect(`Who will get the fastest lap time at ${marketName}?`, allDrivers, openingTS+3, 'F1'),
    buildQuestionSingleSelect(`Who will be the driver of the day at ${marketName}?`, allDrivers, openingTS+4, 'F1'),
    buildQuestionMultipleSelect(`Wich engine will score the most points at ${marketName}?`, allEngines, openingTS+5, 'F1'),
    buildQuestionSingleSelect(`Which tires type has the race's winner at the ${marketName} race end??`, allTires, openingTS+6, 'F1'),
    buildQuestionSingleSelect(`Which driver will receive the DHL Fastest Pit Stop award at ${marketName}?`, allDrivers, openingTS+7, 'F1'),
    buildQuestionTeamMostPoints(allTeams, marketName, openingTS+8),
  ],
